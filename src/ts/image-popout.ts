...
export const rotateImagePopout = () => {
    const popOut = getNextPopout();
    if (!popOut) return;
...
export const doubleFlipImagePopout = () => {
    const popOut = getNextPopout();
    if (!popOut) return;
...
export function setImg(cW: number, cH: number, rotation: number, div: HTMLElement, img?: HTMLImageElement) {
